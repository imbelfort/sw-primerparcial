...
export class SpringCodeGenerator {
  ...
  private extractAttributes(element: joint.dia.Element): Array<{name: string, type: string, isId?: boolean, isRequired?: boolean, isAutoGenerated?: boolean}> {
    ...
    if (!hasExplicitId) {
      // Solo agregar ID automático si no hay atributos o si hay muy pocos atributos
      // Esto evita agregar IDs innecesarios cuando la IA ya creó una entidad completa
      if (attributes.length === 0 || attributes.length <= 2) {
        attributes.unshift({
          name: 'id',
          type: 'Long',
          isId: true,
  ...
  private extractCardinality(link: joint.dia.Link, end: 'source' | 'target'): string {
    // Intentar extraer cardinalidad de diferentes atributos del link
...
${entity.attributes.filter(attr => attr.isRequired && !attr.isId).map(attr => `        this.${attr.name} = ${attr.name};`).join('\n')}
  ...
  private generateEntityMethods(entity: SpringEntity): string {
    ...
    entity.attributes.forEach(attr => {
      ...
      // Solo generar setter si no es un ID auto-generado
      if (!usedMethodNames.has(setterName) && !(attr.isId && attr.isAutoGenerated)) {
